...
def TheCatalogApp():
	...
	return render_template('categories3.html', categories=categories, items = items, STATE = state)	    
...
def gconnect():
    ...
    if stored_access_token is not None and gplus_id == stored_gplus_id:
        response = make_response(json.dumps('Current user is already Shreyanka.'),
                                 200)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def AddNewItem():
	if request.method == 'POST':
	    if login_session['username'] is not None:
			...
			conn = engine.connect()
			select_cat_id = select([Category]).where(Category.name == request.form['Category']) 
			res = conn.execute(select_cat_id)
			for category_id in res:
				newItem = Item(name = request.form['itemname'], description = request.form['description'], cat_id = category_id.id, user_id = login_session.get('user_id'))
				session.add(newItem)
				session.commit()
			conn.close()
			categories = session.query(Category).all()
			items = session.query(Item).order_by(Item.creation_date.desc()).all()
			flash("New Items has been Added")
			return redirect('/catalog')
	else:
	    categories = session.query(Category).all()
...
def ListItems(category_id):
		if request.method == 'GET':
			...
			return render_template('categories3.html', categories=categories, items = items)
...
def UpdateItem(item_id):
	...
	if request.method == 'GET':
		return render_template('UpdateItem.html', items = items, categories = categories, category_name = category_name)
	else:
		if items.user_id == login_session.get('user_id'):
			categories = session.query(Category).filter_by(name=request.form['Category']).one()
			items = session.query(Item).filter_by(id=item_id).one()
